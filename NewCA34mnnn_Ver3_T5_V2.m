...
ds=30;
Rad3Over2 = sqrt(3) / 2;
Xm=xllcorner:ds*1.5:xllcorner+(ncols-1).*cellsize;
Ym=yllcorner+(nrows-1).*cellsize:-ds*1.5:yllcorner;
[Xm,Ym] = meshgrid(Xm,Ym);
n = size(Xm,1);
m = size(Xm,2);
for i=2:2:size(Xm,2)
Ym(:,i) = Ym(:,i) + repmat([ds*Rad3Over2],[n,1]);
end
                    ...
                    switch s
                        case 1
                           if (B(i,j-1)==1 & ~(h(i,j)==0 & h(i,j-1)==0))
                               Qs(i,j,1)= Qij_1(H(i,j),H(i,j-1),h(i,j),h(i,j-1),n,ds,ds*Rad3Over2*2);
                        ...
                        case 3
                           ...
                           Qs(i,j,3)= Qij_1(H(i,j),H(i-1,j),h(i,j),h(i-1,j),n,ds,ds*Rad3Over2*2);
                        ...
                        case 4
                        case 5
                           if (B(i+f,j-1)==1 & ~(h(i,j)==0 & h(i+f,j-1)==0))
                           Qs(i,j,5)= Qij_1(H(i,j),H(i+f,j-1),h(i,j),h(i+f,j-1),n,ds,ds*Rad3Over2*2);
                           if (Qs(i,j,5)<0 && h(i,j)<=e1) || (Qs(i,j,5)>0 && h(i+f,j-1)<=e1)
                               Qs(i,j,5)=0;
                           end
                           else
                               Qs(i,j,5)= 0;
                        ...
                        case 6
                           if (B(i+f,j+1)==1 & ~(h(i,j)==0 & h(i+f,j+1)==0))
                            Qs(i,j,6)= Qij_1(H(i,j),H(i+f,j+1),h(i,j),h(i+f,j+1),n,ds,ds*Rad3Over2*2);
                               if (Qs(i,j,6)<0 && h(i,j)<=e1) || (Qs(i,j,6)>0 && h(i+f,j+1)<=e1)
                                   Qs(i,j,6)=0;
                               end
                           else
                               Qs(i,j,6)= 0;
